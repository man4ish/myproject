...
public class StudySearchFreeFormQueryDelegate implements FreeformStatementDelegate {

    private List<Filter> filters;
    private List<OrderBy> orderBys;
    private String query;

     public StudySearchFreeFormQueryDelegate(){
      // this.query = query; 
    }
    public StudySearchFreeFormQueryDelegate(String query){
       this.query = query; 
    }
    ...
    public StatementHelper getQueryStatement(int offset, int limit)
      ...
      //  System.out.println("my query is: " + query);  // enable this to debug when required
        return sh;
    }
    ...
    public StatementHelper getCountStatement()
        ...
        String queryString = "";
         ...
         int index = this.query.toLowerCase().indexOf("select * ");
         if (index > -1) {
             // Rewrite the where clause
           // return queryString.substring(0, index) + "WHERE " + where + " AND "  + queryString.substring(index + 6);
             queryString = "SELECT COUNT(*) " + this.query.substring(index + 8);   // This function is very very important for complex queries
         }
         
        //StringBuffer query = new StringBuffer("SELECT COUNT(*) FROM study_summary ");
         StringBuffer query = new StringBuffer(queryString);
     //   System.out.println("Reahed count statement: " + query);
        if (filters != null) {
            query.append(QueryBuilder.getWhereStringForFilters(
                    filters, sh));
        }
